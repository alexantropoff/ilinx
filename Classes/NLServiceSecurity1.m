...
{
  if (controlMode == 0)
      [_pcomms send: @"BUTTON PRESS" to: [NSString stringWithFormat: @"%@~%lu", self.serviceName, buttonIndex + 1]];
}

- (void) releaseButton: (NSUInteger) buttonIndex inControlMode: (NSUInteger) controlMode
{
  if (controlMode == 0)
      [_pcomms send: @"BUTTON RELEASE" to: [NSString stringWithFormat: @"%@~%lu", self.serviceName, buttonIndex + 1]];
}

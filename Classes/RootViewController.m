...
- (void) viewSettings: (id) button
{
  ...
  [self presentViewController: configRootController animated: YES completion:nil];
...
- (void) viewWillAppear: (BOOL) animated
{
  ...
  else
  {
    ...
    [_state setObject: [_roomList connectedHost] forKey: kDefaultHostKey];
      [_state setObject: [NSString stringWithFormat: @"%lu", (unsigned long)[_roomList connectedPort]] forKey: kDefaultPortKey];
  }
...
- (void) viewDidAppear: (BOOL) animated
{
  ...
  if (_configStartupType == STARTUP_TYPE_PRESENT_CHOICE)
  {
    ...
    [self presentViewController:profileList animated:YES completion:nil];
    [profileList release];
    _configStartupType = STARTUP_TYPE_AUTO_DETECT;
  }
}

...
{
    [_pcomms send: [NSString stringWithFormat: @"PRESET %lu", preset + 1] to: self.serviceName];
  [self ifNoFeedbackSetCaption: self.serviceName];
}

- (void) savePreset: (NSUInteger) preset withTitle: (NSString *) title
{
    [_pcomms send: [NSString stringWithFormat: @"MENU_UPDATE {{presets}},{{%@}},%lu", title, preset + 1] to: self.serviceName];

...
{
#if LOG_DISCOVERY
    NSLog( @"NLValidateHostOperation %@: Fetching GUI data: http://%@:%lu/gui.xml", self, _host, (unsigned long)_port );
#endif
    _target = [[NSURL URLWithString: [NSString stringWithFormat: @"http://%@:%lu/gui.xml", _host, (unsigned long)_port]] retain];
...
- (NSString *) jsonStringForStatus: (NSUInteger) statusMask withObjects: (BOOL) withObjects
{
  ...
  else
  {
    ...
    NSUInteger currentRoom = (_currentIndex < specialCount)?mainCount:(_currentIndex - specialCount);
      ...
      result = [NSString stringWithFormat: @"{ currentIndex: %lu, length: %ld", (unsigned long)currentRoom, (long)mainCount];
  
    if ((statusMask & JSON_ALL_LOCATIONS) == 0)
        result = [result stringByAppendingFormat: @", %lu: %@", (unsigned long)currentRoom,
                ...
                isOneOff: (BOOL) isOneOff
{
  if (error == nil && [data length] > 0)
  {
    ...
    {
#if LOG_DISCOVERY
        NSLog( @"NLRoomList %@: Successfully parsed GUI data: http://%@:%lu/gui.xml", self, host, (unsigned long)port );
#endif
    ...
    {
#if LOG_DISCOVERY
        NSLog( @"NLRoomList %@: Failed to parse GUI data: http://%@:%lu/gui.xml, error: %@", self, host, (unsigned long)port, error );
#endif
    }
...
#if LOG_DISCOVERY
    if (error != nil)
        NSLog( @"NLRoomList %@: GUI data fetch from http://%@:%lu/gui.xml returned error %@", self, host, (unsigned long)port, error );
    else
    {
        NSLog( @"NLRoomList %@: no GUI data returned from http://%@:%lu/gui.xml", self, host, (unsigned long)port );
    }
#endif          
  }

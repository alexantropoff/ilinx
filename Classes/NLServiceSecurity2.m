    ...
    if (buttonIndex < [currentStates count])
    {
        ...
        (SERVICE_SECURITY_STATE_ENABLED|SERVICE_SECURITY_STATE_HAS_INDICATOR))
      {
        if ((currentState & SERVICE_SECURITY_STATE_INDICATOR_ON) == 0)
            [_pcomms send: [NSString stringWithFormat: @"set BypassOn %lu", buttonIndex + 1] to: self.serviceName];
        else
            [_pcomms send: [NSString stringWithFormat: @"set BypassOff %lu", buttonIndex + 1] to: self.serviceName];
      }
    }
  }
}
